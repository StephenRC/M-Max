...
module ZMotorMount(Qty=1,Bearing=0,RodAdjust=0,X=0,Y=0,Z=0) {
	for(x = [0 : Qty-1]) {
		translate([x*100,0,0]) {
			if(Show) OldMount();
			OriginalBase(X,Y,-Z-52.75,BaseLength/10-1,Bearing); // Z=-52.75 for original position
			ZRodMount(RodAdjust);
		}
	}
}
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
module ZRodMount(Adjust=0,ZRodSize=9.9) {
	difference() {
		translate([BaseWidth/2,102,62]) color("blue") cuboid([60,15,8],rounding=2);
		ZRodMountHoles();
		ZRod(Adjust,ZRodSize);
	}
	ZRodClamp(Adjust,ZRodSize);
}

/////////////////////////////////////////////////////////////////////////////////////////////////////////

module ZRodClamp(Adjust=0,ZRodSize=9.9) {
	difference() {
		union() {
			translate([BaseWidth/2,102,70]) color("gray") cuboid([40,15,8],rounding=2);
			difference() {
				translate([BaseWidth/2,102,70]) color("green") rotate([90,0,0]) cyl(h=15,d=ZRodSize*2,rounding=2);
				translate([BaseWidth/2,102,64]) color("purple") cuboid([ZRodSize*3,20,15]);
			}
		}
...
module ZRod(Adjust=0,Size=9.9) {
	translate([BaseWidth/2,101,65+Adjust]) color("cyan") rotate([90,0,0]) cyl(h=50,d=9.9); // Z Rod
}

//////////////////////////////////////////////////////////////////////////////////////////////////////

module ZRodMountHoles(Screw) {
	translate([BaseWidth/2-10,103,70]) color("white") cyl(h=50,d=screw5);
	translate([BaseWidth/2+10,103,70]) color("lightgray") cyl(h=50,d=screw5);
	translate([BaseWidth/2-10,103,76]) color("lightgray") cyl(h=5,d=screw5hd);
	translate([BaseWidth/2+10,103,76]) color("white") cyl(h=5,d=screw5hd);
...
module OriginalBase(X=0,Y=0,Z=0,Z2=0,Length=100,Bearing=0) { // makes the motor mount holder
	echo(Length);
	difference() {
		union() {
			color("skyblue") cuboid([BaseWidth,BaseThickness,BaseThickness],rounding=0,p1=[0,0]);
			OriginalSideBase(X,Y,Z,0,BaseLength);
			base(X,Y,Z,Z2,Length);
		}
		BaseHoles();
...
module base(X=0,Y=0,Z=0,Z2=0,BaseLength=50) { // makes the motor mount holder
	difference() {
		union() {
			//color("skyblue") cuboid([BaseWidth,BaseLength,Thickness-2],rounding=1,p1=[0,0]);
			SideBase(X,Y,Z);
			SideBase(X,Y+90,Z,5);
...
module OriginalSideBase(X=0,Y=0,Z=0,Add=0,Length) { // prevent the base from flexing in between the 2020 pieces
	translate([X+14.5,Y+5,0]) color("firebrick") cuboid([Thickness,Length-6+Add,21],rounding=2,p1=[0,0]);
	translate([X+BaseWidth-21.5,Y+5,0]) color("seashell") cuboid([Thickness,Length-6+Add,21],rounding=2,p1=[0,0]);
}
