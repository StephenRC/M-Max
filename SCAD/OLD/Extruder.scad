...
module FanMountHoles(Screw=Yes3mmInsert(UseLargeInsert),Left=1) {	// fan mounting holes
	ScrewL=25;
	if(Left) {
...
module TitanExtruderPlatform(recess=3,InnerSupport=0,MountingHoles=1) { // extruder platform for e3d titan with (0,1)BLTouch or
	difference() {										// (2)Proximity or (3)dc42's ir sensor
		...
		translate([-50,0,44.5]) rotate([90,0,90]) FanMountHoles(Yes3mmInsert(UseLargeInsert)); // mounting holes for bltouch & prox sensor
		translate([-10,70,0]) IRMountHoles(Yes3mmInsert(UseLargeInsert)); // mounting holes for irsensor bracket
...
module PCFanMounting(Screw=screw3) {
	if(Use3mmInsert) {
		translate([-extruder/2+54,-heightE/2 - 1.8*wall,heightE - extruder_back - PCfan_spacing/2 + fan_offset])
			rotate([0,90,0]) color("red") cylinder(h = 20,d = Screw);
		translate([-extruder/2+54,-heightE/2 - 1.8*wall,heightE - extruder_back + PCfan_spacing/2 + fan_offset])
			rotate([0,90,0]) color("blue") cylinder(h = 20,d = Screw);
	}
...
module IRMountHoles(Screw=screw3) // ir screw holes for mounting to extruder plate
{
	if(Use3mmInsert) {
		translate([Spacing+ShiftIR+ShiftHotend,-107,0]) rotate([90,0,0]) color("blue")
			cylinder(h=screw3inl*2,d=Screw);
