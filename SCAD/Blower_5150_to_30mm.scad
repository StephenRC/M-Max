...
module blower_adapter(blower_h,blower_w,blower_m_dist) { // to use a 50mm 10x15 blower instead of a 40mm axial
	...
	difference() {
		...
		translate([screw4/2+15-blower_w+fan_adjustLR,screw4+blower_w-13.2,blower_m_dist-1]) rotate([90,0,90])
			cylinder(h=10,d=screw4t);
		translate([4+fan_adjustLR,4,3]) color("white") cylinder(h=bthick*2,r=screw3hd/2);
	}
	difference() {
		translate([17.7-blower_w+fan_adjustLR,blower_h-9.5,blower_m_dist-18+bthick]) rotate([30,0,0]) color("cyan")
		cubeX([screw4+1,screw4+6,20],2);
		translate([16.7-blower_w+fan_adjustLR,blower_h-8,blower_m_dist-20+bthick]) color("pink") cube([screw4+3,screw4+4,20],2);
		translate([screw4/2+15-blower_w+fan_adjustLR,screw4+blower_w-13.3,blower_m_dist+6.9]) rotate([90,0,90])
			cylinder(h=10,d=screw4t);
	}
}
