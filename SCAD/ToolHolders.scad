...
module PuttyKnifeHolder() {
	difference() {
		color("cyan") hull() {
			cuboid([60,20,4],rounding=2);
			translate([0,-6,25]) cuboid([20,10,4],rounding=2);
		}
		translate([0,4.5,13]) color("blue") cuboid([54,1.5,35]);
		translate([0,0,17]) rotate([90,0,0]) color("red") cyl(h=30,d=screw5);
		translate([0,6,17]) rotate([90,0,0]) color("green") cyl(h=15,d=screw5hd);
	}
}
