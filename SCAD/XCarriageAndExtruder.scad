///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
// XCarriageAndExtruder.scad - x carriage for makerslide
// created: 2/3/2014
...
//CarridgeAllInOne(0,1,2);	// Clamps,Loop,Titan -- Not used with MMAX
...
module Carriage_v2(Titan=0,Tshift=0,Clamps=0,Loop=0,DoBeltDrive=0,Rear=0,MoveBeltLoops=0,ExtMountingHoles=1) { // extruder side
	difference() {
		union() {
			...
			color("blue") cubeX([width,HorizontalCarriageHeight,wall],1);
				translate([VerticalCarriageWidth/2+31,HorizontalCarriageHeight+0.9,0]) roundedinner();
			translate([VerticalCarriageWidth/2+6.5,HorizontalCarriageHeight+0.9,8]) rotate([0,180,0]) roundedinner();
		}
		BeltDriveNotch(Loop);
		// wheel holes
...
module CarriageExtruderPlateNuts(){
	if(Yes3mmInsert() == screw3) {
		translate([5,HorizontalCarriageHeight+3,nut3/2+1]) rotate([90,0,0]) color("black") nut(nut3,5);
		translate([widthE/2,HorizontalCarriageHeight-10,nut3/2+1]) rotate([90,0,0]) color("white") nut(nut3,5);
		translate([widthE-5,HorizontalCarriageHeight+3,nut3/2+1]) rotate([90,0,0]) color("gray") nut(nut3,5);
	}
}
...
module ExtruderMountHolesFn(Screw=Yes3mmInsert(),Fragments=100,All=0) {
...
module ExtruderPlatform(recess=5) // bolt-on extruder platform
...
module TitanExtruderPlatform(recess=5,InnerSupport=0,MountingHoles=1) { // extruder platform for e3d titan with (0,1)BLTouch or
	//echo("s",InnerSupport);
	difference() {										// (2)Proximity or (3)dc42's ir sensor
		...
		translate([-13,20,45]) rotate([90,0,0]) PCFanMounting(Yes3mmInsert());
...
module LEDRingMount(Screw=screw5) {
	translate([30.5+ShiftHotend2,-24+ShiftHotend,-10]) color("gray") cylinder(h=20,d=Screw);		// mounting hole for a holder
	if(Screw==screw5) translate([30.5+ShiftHotend2,-24+ShiftHotend,2]) color("black") nut(nut5,5);		// with a 75mm circle led
	translate([30.5+ShiftHotend2,12.5+ShiftHotend,-10]) color("lightgray") cylinder(h=20,d=Screw);		// mounting hole holder
	if(Screw==screw5) translate([30.5+ShiftHotend2,12.5+ShiftHotend,2.5]) color("black") nut(nut5,5);	// with a 75mm circle led
}
...
module PCFanMounting(Screw=Yes3mmInsert()) {
	translate([-extruder/2+54,-heightE/2 - 1.8*wall,heightE - extruder_back - PCfan_spacing/2 + fan_offset])
		rotate([0,90,0]) color("red") cylinder(h = GetHoleLen3mm(Screw),d = Screw);
	translate([-extruder/2+54,-heightE/2 - 1.8*wall,heightE - extruder_back + fan_spacing/2 + fan_offset])
		rotate([0,90,0]) color("blue") cylinder(h = GetHoleLen3mm(Screw),d = Screw);
}
...
{
	translate([Spacing+ShiftIR+ShiftHotend,-106,0]) rotate([90,0,0]) color("blue")
		cylinder(h=GetHoleLen3mm(Screw),d=Screw);
	translate([ShiftIR+ShiftHotend,-106,0]) rotate([90,0,0])
...
///////////////////end of XCarriageAndExtruder.scad ///////////////////////////////////////////////////////////////////////
